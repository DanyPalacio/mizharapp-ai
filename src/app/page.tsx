  ...
  const items = [
    {
      ...
      icon: (
        <svg width="28" height="28" viewBox="0 0 28 28" fill="none">
          <rect width="28" height="28" rx="7" fill="#F5F6F7"/>
          <path d="M8 20V8h4l2 4 2-4h4v12M12 12h8" stroke="var(--mz-graphite)" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
      ),
    },
    {
      label: "BUILT-IN",
      name: "McKinsey Frameworks",
      icon: (
        <svg width="28" height="28" viewBox="0 0 28 28" fill="none">
          <rect width="28" height="28" rx="7" fill="#F5F6F7"/>
          <rect x="7" y="15" width="4" height="6" rx="1" fill="var(--mz-orange)"/>
          <rect x="12" y="11" width="4" height="10" rx="1" fill="var(--mz-graphite)"/>
          <rect x="17" y="7" width="4" height="14" rx="1" fill="var(--mz-orange)"/>
        </svg>
      ),
    },
    {
      ...
      icon: (
        <svg width="28" height="28" viewBox="0 0 28 28" fill="none">
          <rect width="28" height="28" rx="7" fill="#F5F6F7"/>
          <path d="M7 14a7 7 0 1 0 14 0" stroke="var(--mz-graphite)" strokeWidth="1.6" strokeLinecap="round"/>
          <path d="M21 14l-3-3m3 3l-3 3" stroke="var(--mz-orange)" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
      ),
    },
    {
      ...
      icon: (
        <svg width="28" height="28" viewBox="0 0 28 28" fill="none">
          <rect width="28" height="28" rx="7" fill="#F5F6F7"/>
          <path d="M7 19c2-3 4-5 7-4s5-3 7-6" stroke="var(--mz-orange)" strokeWidth="1.8" strokeLinecap="round" fill="none"/>
          <circle cx="21" cy="9" r="2" fill="var(--mz-orange)"/>
        </svg>
      ),
    },
  ]
        ...
        {items.map(item => (
          <div key={item.name} style={{ display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div>
              ...
              <div style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 15, color: "var(--mz-ink)", marginTop: 2 }}>{item.name}</div>
            </div>
          </div>
        ))}
      </div>
        ...
        <div style={{ marginTop: 56, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, maxWidth: 720, margin: "56px auto 0" }}>
          {[
            ...
            <div key={p.plan} style={{ background: p.ctaStyle === "primary" ? "white" : "rgba(255,255,255,0.05)", border: p.ctaStyle === "primary" ? "none" : "1px solid rgba(255,255,255,0.1)", borderRadius: 16, padding: 32, textAlign: "left" }}>
              ...
              <Link href={p.ctaStyle === "primary" ? "/login?intent=pro" : "/onboarding"} className={`btn btn--${p.ctaStyle} btn--lg`} style={{ marginTop: 28, width: "100%", justifyContent: "center" }}>
                {p.cta}
              </Link>
            </div>
          ))}
